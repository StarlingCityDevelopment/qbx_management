...
local function addMenuItem(menuItem)
    ...
    if not menuItem.args.type then
        return
    end
...
local function manageEmployee(player, groupName, groupType)
    ...
    for groupGrade, gradeTitle in pairs(employeeLoop) do
        employeeMenu[#employeeMenu + 1] = {
            ...
            onSelect = function()
                lib.callback.await(
                    "qbx_management:server:updateGrade",
                    false,
                    player.cid,
                    player.grade,
                    tonumber(groupGrade),
...
local function employeeList(groupType)
    ...
    for _, employee in pairs(employees) do
        local employeesData = {
            title = employee.name,
            description = groupType == "job" and JOBS[groupName].grades[employee.grade].name
                or GANGS[groupName].grades[employee.grade].name,
            onSelect = function()
        ...
        if employee.hours and employee.last_checkin then
            employeesData.metadata = {
                {
                    label = locale("menu.employee_status"),
                    value = employee.onduty and locale("menu.on_duty") or locale("menu.off_duty"),
                },
...
function OpenBossMenu(groupType)
    if
        groupType ~= "gang" and groupType ~= "job"
        or not QBX.PlayerData[groupType].name
        or not QBX.PlayerData[groupType].isboss
    then
        return
    end
    ...
    lib.registerContext({
        id = "openBossMenu",
        title = groupType == "gang" and string.upper(QBX.PlayerData.gang.label)
            or string.upper(QBX.PlayerData.job.label),
        options = bossMenu,
        onExit = function()
...
local function createZone(zoneInfo)
    lib.zones.sphere({
        ...
        onExit = function()
            exports.sleepless_interact:removeCoords(self.interact)
        end,
    })
end
